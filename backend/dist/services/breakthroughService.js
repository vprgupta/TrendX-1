...
exports.getBreakthroughs = getBreakthroughs;
exports.startBreakthroughRefresher = startBreakthroughRefresher;
...
function isWithinAge(pubDate) {
    const ms = new Date(pubDate).getTime();
    if (isNaN(ms))
        return false;
    return Date.now() - ms <= MAX_AGE_HOURS * 3600000;
}
async function fetchDomain(domain) {
    ...
    if (hit)
        return hit;
    const results = await Promise.allSettled(domain.feeds.map(url => parser.parseURL(url)));
    const items = [];
    for (const r of results) {
        ...
        for (const item of feed.items) {
            ...
            items.push({
                ...
                source: item.creator || item.source?.title || feed.title || domain.label,
...
async function getBreakthroughs() {
    const allResults = await Promise.allSettled(DOMAINS.map(d => fetchDomain(d)));
    const all = [];
 ...
 */
function startBreakthroughRefresher() {
