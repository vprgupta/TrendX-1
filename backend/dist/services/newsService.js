...
exports.getLocalNews = exports.getNews = exports.getTechNews = exports.getWorldNews = void 0;
const rss_parser_1 = __importDefault(require("rss-parser"));
const node_cache_1 = __importDefault(require("node-cache"));
const breakingNewsService_1 = require("./breakingNewsService");
// 5-minute cache — politics news must be fresh
const newsCache = new node_cache_1.default({ stdTTL: 300 });
 ...
 */
const getWorldNews = async (category, country = 'us') => {
    ...
    if (NEWSDATA_API_KEY) {
        try {
            ...
            if (data.status === 'success' && data.results) {
                newsItems = data.results.map((article) => ({
    ...
    for (const { item } of scored) {
        ...
        const isDupe = seen.some(seenTitle => {
            ...
            let overlap = 0;
            titleWords.forEach(w => { if (seenWords.has(w))
                overlap++; });
            return overlap >= 3;
        });
        if (!isDupe) {
...
exports.getWorldNews = getWorldNews;
/**
 * Get technology news from various RSS feeds
 */
const getTechNews = async (category) => {
...
exports.getTechNews = getTechNews;
/**
 ...
 */
const getRegionalNews = async (category, countryCode) => {
    const cc = countryCode.toUpperCase();
    const cat = category.toLowerCase();
    // Map loose category names to our keys
    const catKey = (() => {
        if (['politics', 'nation', 'top', 'local'].includes(cat))
            return 'politics';
        if (['business', 'finance', 'economy'].includes(cat))
            return 'business';
        if (['health', 'medicine'].includes(cat))
            return 'health';
        if (['entertainment', 'bollywood', 'movies'].includes(cat))
            return 'entertainment';
        if (['sports', 'cricket', 'football'].includes(cat))
            return 'sports';
        if (['science', 'tech', 'technology'].includes(cat))
            return 'science';
        return null;
    })();
        ...
        for (const r of results) {
            ...
            for (const item of r.value.items.slice(0, 20)) {
                regionalItems.push({
                    ...
                    content: item.content ?? item.contentSnippet ?? '',
                    contentSnippet: item.contentSnippet ?? '',
                    source: item.creator ?? item.source?.title ?? r.value.title ?? 'Local',
                    imageUrl: extractItemImage(item),
                    author: item.creator ?? item.author,
                });
            }
    ...
    const [genericResult] = await Promise.allSettled([(0, exports.getWorldNews)(catKey ?? cat, cc)]);
    ...
    if (deduped.length > 0)
        newsCache.set(cacheKey, deduped, 300); // 5-min cache
    return deduped;
};
/**
 * Smart router — each category gets the best specialized source
 */
const getNews = async (category = 'world', country = 'US') => {
    ...
    return (0, exports.getWorldNews)(targetCategory, country);
};
 ...
 */
const getLocalNews = async (state, city = '', category = 'general') => {
    ...
    const raw48h = Date.now() - 48 * 3600000;
    ...
    for (const r of results) {
        ...
        for (const item of r.value.items) {
            ...
            if (isNaN(pubMs) || pubMs < raw48h)
                continue; // older than 48h → skip
            const text = `${item.title} ${item.contentSnippet || ''}`.toLowerCase();
            // STRICT: article must mention the state or one of its cities/keywords
            if (!verifyKws.some(kw => text.includes(kw)))
                continue;
            allItems.push({
                title: item.title ?? 'Untitled',
                link: item.link ?? '#',
                ...
                contentSnippet: item.contentSnippet ?? '',
                source: item.source?.title ?? item.creator ?? r.value.title ?? state,
                imageUrl: extractItemImage(item),
                author: item.creator ?? item.author,
            });
        }
...
exports.getLocalNews = getLocalNews;
/**
 * Google News RSS fallback
 */
const getGoogleNewsRSS = async (category, country) => {
 ...
 */
const getHackerNewsTrending = async (subCategory = 'tech') => {
    ...
    try {
        ...
        if (data.hits && data.hits.length > 0) {
            const newsItems = data.hits
                .filter((hit) => hit.url) // Only include stories with external links
                .map((hit) => ({
 ...
 */
const getTechRSSFallback = async (subCategory) => {
    const feeds = {
        'ai': 'https://www.artificialintelligence-news.com/feed/',
        'mobile': 'https://www.theverge.com/rss/mobile/index.xml',
        'cybersecurity': 'https://www.darkreading.com/rss.xml',
 ...
 */
const getGuardianNews = async (section = 'world', country = 'US') => {
    ...
    try {
        ...
        if (data.response && data.response.results) {
            const newsItems = data.response.results.map((article) => ({
 ...
 */
const getGlobalPoliticsNews = async (country = 'US') => {
    ...
    const MAX_AGE_H = 24;
    const cutoffMs = Date.now() - MAX_AGE_H * 3600000;
    const isFresh = (pubDate) => {
        const ms = new Date(pubDate).getTime();
        return !isNaN(ms) && ms >= cutoffMs;
    };
    // ── Fetch high-volume and high-quality sources in parallel ────────────────
    const [guardianResult, breakingResult, newsDataResult, googlePoliticsResult] = await Promise.allSettled([
        ...
        (0, breakingNewsService_1.getBreakingNews)('World'),
        // 3. NewsData.io (via our existing getWorldNews for high volume)
        (0, exports.getWorldNews)('world', country),
...
const extractItemImage = (item) => {
    ...
    if (mg && mg['media:content']) {
        ...
        if (mgContent.$ && mgContent.$.url)
            return mgContent.$.url;
    }
